...
class UpworkQuerier: 
    def __init__(self):
        self.log = open('log_upwork_job_collection_2018_04_03_unitedstates.txt', 'a') # Creating a log
    ...
    def collect_jobs(self): 
        ...
        self.data = {'q': 'united states'} # Parameters for searching for freelancers (use 'q': 'a' if want to search all freelancers)

        while True:
            print "Now about to take jobs at offset {0}".format(self.offset)
            try: 
                print "Now about to sleep before we collect jobs again"
                time.sleep(61)
                jobs_on_page = self.client.provider_v2.search_jobs(data=self.data, page_offset=self.offset, page_size=self.size_of_page)
                ...
                print "Storing job into database"
                self.on_job(jobs_on_page)
                self.offset += self.size_of_page

    def on_job(self, jobs):
        for job in jobs:
            contractor_id = job['id']
            ...
            try:
                ...
                if(job['client']['country'] == "United States"):
                    print "American"
                    self.cur.execute("INSERT INTO upwork_jobs_unitedstates_2018_04_03 (contractor_id, job) VALUES (%s, %s);",
                                [contractor_id, psycopg2.extras.Json(job)])
                    self.conn.commit()
                else:
                    print "Not American"
                ...
                print(err)
                self.log.write("Failed to parse job contractor: " + contractor_id + " because of {0}".format(err) + "\n")
                self.log.flush()
                print "Failed to parse job contractor: " + contractor_id + "\n"

myObject = UpworkQuerier()
myObject.collect_jobs()